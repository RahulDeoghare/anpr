...
def initialize_paddleocr():
    """
    Initialize PaddleOCR with CPU mode for stability (GPU causes CUDNN issues)
    """
    global ocr_instance, ocr_gpu_mode, ocr_gpu_failed
    
    print("[INFO] Initializing PaddleOCR...")
    
    # Start with CPU mode for stability due to persistent CUDNN issues
    print("[INFO] Using CPU mode for PaddleOCR to avoid CUDNN errors...")
    try:
        ocr = PaddleOCR(use_textline_orientation=True, lang='en', use_gpu=False, show_log=False)
        print(f"[INFO] PaddleOCR initialized successfully with GPU: False (CPU mode for stability)")
        ocr_gpu_mode = False
        ocr_gpu_failed = False  # Not failed, just using CPU by choice
        return ocr
    except Exception as e:
        print(f"[WARNING] Failed to initialize PaddleOCR with CPU mode: {e}")
    
    # If standard CPU fails, try with minimal configuration
    try:
